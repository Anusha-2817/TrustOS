...
- **Python environment** — the project venv is `.venv/` at the repo root (made with uv, Python 3.13.7). Phase 3 fixed it: it had fastapi/pydantic/openai but **not numpy, scikit-learn or pytest**, so the ML code only ran on the system Python 3.14. Now `.venv/Scripts/python -m pip install -r backend/requirements-dev.txt` (= `requirements.txt` + pytest) installs everything; verified: numpy 2.5.3, scikit-learn 1.9.1 reproduce the Phase 2 numbers exactly (360/67/23). Use the venv, not system Python.
...
Run from `backend/`: `../.venv/Scripts/python -m pytest tests` (~35 s; 84 pass + 1 skipped recorder). `conftest.py` forces a dummy `OPENAI_API_KEY` and replaces `main.call_llm` with a deterministic fake, so no test touches the network.
...
- `test_product_risk.py` — the full 3×3 `escalate_tier` matrix, the adapter, lazy import (subprocess), and end-to-end escalation through the routes. It uses catalogue fixtures `B0BYYPTLHX` (ML HIGH), `B0014LE00U` (MEDIUM) and `B07FSDFF75` (LOW), whose levels are re-asserted.
